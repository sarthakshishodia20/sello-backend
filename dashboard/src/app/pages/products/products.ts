...
export class ProductsComponent implements OnInit {
  ...
  // Top Selling Module States
  topSellingModalVisible = signal(false);
  topSellingTab = signal<'add' | 'remove'>('add');
  topSellingSearchQuery = '';
  topSellingFilteredItems = signal<any[]>([]);
  topSellingSelectedItems = new Set<number>();
  topSellingOffset = 0;
  topSellingTotalRecords = signal(0);
  ...
  // ─── Top Selling Curation Methods ───
  openTopSellingManager() {
    this.topSellingTab.set('add');
    this.topSellingSelectedItems.clear();
    this.topSellingSearchQuery = '';
    this.topSellingOffset = 0;
    this.loading.set(true);
    this.loadTopSellingList();
    this.topSellingModalVisible.set(true);
  }

  switchTopSellingTab(tab: 'add' | 'remove') {
    this.topSellingTab.set(tab);
    this.topSellingSelectedItems.clear();
    this.topSellingOffset = 0;
    this.loading.set(true);
    this.loadTopSellingList();
  ...
  onTopSellingSearch(query: string) {
    this.topSellingSearchQuery = query;
    this.topSellingOffset = 0;
    this.loading.set(true);
    this.loadTopSellingList();
  }

  onTopSellingScroll(event: any) {
    const el = event.target;
    if (el.scrollHeight - el.scrollTop <= el.clientHeight + 10) {
      if (!this.loading() && this.topSellingFilteredItems().length < this.topSellingTotalRecords()) {
        this.topSellingOffset += 25;
        this.loading.set(true);
        this.loadTopSellingList();
      }
    }
  ...
  loadTopSellingList() {
    const search = (this.topSellingSearchQuery || '').trim();

    if (this.topSellingOffset === 0) {
      this.topSellingFilteredItems.set([]);
    }

    const params: any = {
      include_unavailable: 'true',
      limit: 25,
      offset: this.topSellingOffset
    };
    if (search) params.search = search;
    ...
    this.api.get<any>('/products/inherited', params).subscribe({
      next: (res) => {
        ...
        const list = rawProducts.map((p: any) => ({
          id: p.catalogue_id,
          name: p.effective_name || p.master_name,
          is_top_selling: p.is_top_selling
        }));

        const filteredList = list.filter((item: any) => {
          if (this.topSellingTab() === 'add') {
            return !item.is_top_selling;
          } else {
            return !!item.is_top_selling;
          }
        });
        ...
        if (this.topSellingOffset === 0) {
          this.topSellingFilteredItems.set(filteredList);
        } else {
          this.topSellingFilteredItems.set([...this.topSellingFilteredItems(), ...filteredList]);
        }
        
        this.topSellingTotalRecords.set(res.data.total || list.length);
        this.loading.set(false);
      },
      error: () => {
        this.loading.set(false);
        this.messageService.add({ severity: 'error', summary: 'Failed', detail: 'Could not load products.' });
      }
    });
  }

  toggleTopSellingItemSelection(item: any) {
    if (this.topSellingSelectedItems.has(item.id)) {
      this.topSellingSelectedItems.delete(item.id);
    } else {
      this.topSellingSelectedItems.add(item.id);
    }
  }

  isTopSellingItemSelected(id: number): boolean {
    return this.topSellingSelectedItems.has(id);
  }

  removeTopSellingItemSelection(id: number) {
    this.topSellingSelectedItems.delete(id);
  }

  getSelectedTopSellingItemsList(): any[] {
    return Array.from(this.topSellingSelectedItems).map(id => {
      const found = this.topSellingFilteredItems().find(i => i.id === id);
      return found || { id, name: '(item #' + id + ')' };
    });
  }

  executeTopSellingAction() {
    if (this.topSellingSelectedItems.size === 0) return;
    this.loading.set(true);

    const ids = Array.from(this.topSellingSelectedItems);
    const isTopSelling = this.topSellingTab() === 'add';

    this.api.post('/products/top-selling', { catalogueIds: ids, is_top_selling: isTopSelling }).subscribe({
      next: () => {
        this.loading.set(false);
        this.topSellingModalVisible.set(false);
        this.topSellingSelectedItems.clear();
        this.messageService.add({
          severity: 'success',
          summary: isTopSelling ? 'Added' : 'Removed',
          detail: `${ids.length} item(s) ${isTopSelling ? 'added to' : 'removed from'} Top Selling list successfully.`
        });
        this.offset = 0;
        this.loadProducts();
      },
      error: (err) => {
        this.loading.set(false);
        this.messageService.add({ severity: 'error', summary: 'Failed', detail: err.error?.message || 'Could not update top selling items.' });
      }
    });
  }
}
